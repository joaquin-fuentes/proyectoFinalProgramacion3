...
import { obtenerProductos, obtenerVenta, consultaEditarVenta, consultaEditarProducto } from "../../helpers/queries";
...
const EditarVenta = () => {
    ...
    useEffect(() => {
        obtenerVenta(id).then((respuesta) => {
            if (respuesta != null) {
                setValue("fechaVenta", respuesta.fechaVenta)
                setValue("nombreCliente", respuesta.nombreCliente)
                setValue("nombreVendedor", respuesta.nombreVendedor)
                setImporteTotal(respuesta.importeTotal)
                setProductosVendidos(respuesta.productosVendidos)
            } else {
                Swal.fire(
                    "Error",
                    "No se pudo obtener los datos de la API",
    ...
    const onSubmit = (datos) => {
        ...
        consultaEditarVenta(nuevaVenta, id).then((respuesta) => {
            if (respuesta.status === 200) {
                Swal.fire(
                    'Venta Editada!',
                    `La nueva venta fue actualizada con exito`,
                    'success'
                )
    ...
    const buscarStockProductoCoincidente = (productoVendido) => {
        const productoCoincidente = productos.find(
            (productoDB) => productoDB.nombreProducto === productoVendido.nombreProducto
        );
        if (productoCoincidente) {
